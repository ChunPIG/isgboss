...
LOCAL_DIR=/data/msplogs/logs/virus
REMOTE_DIR=/data/msplogs/logs/virus

SHELL_HOME=/opt/isg/boss/shell

LOG_HOME=$SHELL_HOME/shell-logs
FTP_LOG=$LOG_HOME/ftp50virus.log
...
do
   ...
   TRANSFER_LOG=$LOG_HOME/'ftp50virus-'$(date -d  today +%Y%m%d).log
...
then
  ...
  RUN_LOG=$LOG_HOME/ftp50virus.$timestamp.log
...
bye
\" |ftp -v -n |tee ${FTP_LOG} |sed 's/^/>/g' >> ${RUN_LOG}
" >$SHELL_HOME/mrename.sh

sh $SHELL_HOME/mrename.sh
...
rm $SHELL_HOME/mput.sh
rm $SHELL_HOME/mrename.sh
