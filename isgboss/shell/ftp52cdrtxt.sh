...
FTPHOST=192.168.1.52
USER=ecgwuser
PSWD=ecgw123

LOCAL_DIR=/data/msplogs/logs/cdrtxtdest
REMOTE_DIR=/data/msplogs/logs/cdrtxt

SHELL_HOME=/opt/isg/boss/shell

LOG_HOME=$SHELL_HOME/shell-logs
FTP_LOG=$LOG_HOME/ftp52cdr.log
...
do
   ...
   TRANSFER_LOG=$LOG_HOME/'ftp52cdr-'$(date -d  today +%Y%m%d).log
...
then
  ...
  RUN_LOG=$LOG_HOME/ftp52cdr.$timestamp.log
