...
public class OcsEndpointImpl implements InitializingBean
{
	private static final Logger LOG = LoggerFactory.getLogger( OcsEndpointImpl.class ) ;
	AtomicLongArray counters = new AtomicLongArray(10) ;
	int batchSize = 100 ;
	String source = "ocs" ;
	String inputDir = "./data/ocs/input";
	String backupDir = "./data/ocs/backup";
	String outputDir = "./data/ocs/output";
	Matcher matcher ;
	...
	public void process( File file )
	{
		...
		try
		{
			...
			String msgType = source + "." + lowercaseFileName.substring(0,lowercaseFileName.length()-23 ) ;
	...
	public String process( String line, String msgType )
	{
		...
		String[] fields = StringUtils.split( line, fieldSep, 6 ) ;
		
		LOG.debug( "MatchTrace-Ocs Endpoint line content {} , Split fields length is {}", line, fields.length );
		
		MediaMessage mediaMessage = new MediaMessage() ;
		mediaMessage.setSource( source ) ;
		mediaMessage.setMsgType( msgType ) ;
		mediaMessage.setSrcId( fields[1] ) ;
		mediaMessage.setDestId( fields[2] ) ;
		mediaMessage.setContent( fields[5] ) ;
		...
		buffer.append( fields[1] ).append( fieldSep ) ;
		buffer.append( fields[2] ).append( fieldSep ) ;
		buffer.append( fields[3] ).append( fieldSep ) ;
		buffer.append( fields[4] ).append( fieldSep ) ;
